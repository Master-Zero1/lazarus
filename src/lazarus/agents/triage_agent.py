...
DIRECTIVE_PATH = PACKAGE_ROOT / "directives" / "triage_issues_and_prs.md"
CATEGORIES = (
    "duplicate/resolved",
    "obsolete",
    "still valid",
...
def _read_snapshot(path: Path, expected_resource: str) -> dict[str, Any]:
    ...
    if expected_resource in payload and not isinstance(payload[expected_resource], list):
        raise RuntimeError(
            f"Snapshot {path} has malformed `{expected_resource}` data: expected a list when present, got {type(payload[expected_resource]).__name__}."
        )
    if expected_resource == "pull_requests" and "detailed_pull_requests" in payload and not isinstance(
        payload["detailed_pull_requests"], list
    ):
        raise RuntimeError(
            f"Snapshot {path} has malformed `detailed_pull_requests` data: expected a list when present, got {type(payload['detailed_pull_requests']).__name__}."
        )
    if expected_resource == "pull_requests":
        for index, detail in enumerate(payload.get("detailed_pull_requests", [])):
            if not isinstance(detail, dict):
                continue
            for field in ("merge_status", "comment_counts"):
                if field in detail and not isinstance(detail[field], dict):
                    raise RuntimeError(
                        f"Snapshot {path} has malformed `detailed_pull_requests[{index}].{field}` data: "
                        f"expected an object when present, got {type(detail[field]).__name__}."
                    )
    return payload
...
def _pull_request_record(pull_request: dict[str, Any], detail: dict[str, Any] | None, as_of: date) -> dict[str, Any]:
    ...
    description_value = source.get("description") if detail else source.get("body")
    ...
    if state != "open":
        ...
        confidence = "high" if detail is not None and (bool(description.strip()) or merge_status is not None) else "medium"
...
def _snapshot_query_state(snapshot: dict[str, Any]) -> str | None:
    """Read the declared fetch scope without inferring it from item states."""
    query = snapshot.get("query")
    state = query.get("state") if isinstance(query, dict) else None
    return state.casefold() if isinstance(state, str) else None


def _exclude_closed_for_default_scope(
    items: list[dict[str, Any]],
    resource_label: str,
    query_state: str | None,
    include_closed: bool,
) -> tuple[list[dict[str, Any]], list[str]]:
    """Honor the directive's open-only default when a snapshot intentionally includes closed rows."""
    if include_closed or query_state not in {"all", "closed"}:
        return items, []
    included = [
        item
        for item in items
        if not (isinstance(item.get("state"), str) and item["state"].casefold() == "closed")
    ]
    excluded_count = len(items) - len(included)
    return included, [
        "The {0} snapshot declares query state `{1}`, but --include-closed was not supplied; {2} closed item(s) were excluded to honor the directive's default open-only scope.".format(
            resource_label, query_state, excluded_count
        )
    ]
...
def triage(
    ...
    prs_snapshot: dict[str, Any],
    as_of: date,
    health_context: dict[str, Any],
    include_closed: bool = False,
) -> tuple[list[dict[str, Any]], list[str], list[str]]:
    """Classify every supplied issue and PR without performing a remote mutation."""
    raw_issues = issues_snapshot.get("issues", [])
    raw_pull_requests = prs_snapshot.get("pull_requests", [])
    raw_details = prs_snapshot.get("detailed_pull_requests", [])
    issues = [item for item in raw_issues if isinstance(item, dict)]
    pull_requests = [item for item in raw_pull_requests if isinstance(item, dict)]
    detailed_pull_requests = [item for item in raw_details if isinstance(item, dict)]
    snapshot_warnings: list[str] = []
    excluded_issues = len(raw_issues) - len(issues)
    excluded_pull_requests = len(raw_pull_requests) - len(pull_requests)
    excluded_details = len(raw_details) - len(detailed_pull_requests)
    if excluded_issues:
        snapshot_warnings.append(f"{excluded_issues} issue(s) in the snapshot were not valid objects and were excluded.")
    if excluded_pull_requests:
        snapshot_warnings.append(f"{excluded_pull_requests} pull request(s) in the snapshot were not valid objects and were excluded.")
    if excluded_details:
        snapshot_warnings.append(
            f"{excluded_details} detailed pull request record(s) in the snapshot were not valid objects and were excluded."
        )
    issues, issue_scope_warnings = _exclude_closed_for_default_scope(
        issues, "issues", _snapshot_query_state(issues_snapshot), include_closed
    )
    pull_requests, pull_request_scope_warnings = _exclude_closed_for_default_scope(
        pull_requests, "pull requests", _snapshot_query_state(prs_snapshot), include_closed
    )
    details = {
        detail.get("number"): detail
        for detail in detailed_pull_requests
        if isinstance(detail.get("number"), int)
    }
    ...
    return records, snapshot_warnings, [*issue_scope_warnings, *pull_request_scope_warnings]
...
def _render_report(
    ...
    lines = [
        ...
        f"- Closed items included: `{include_closed}`",
        ...
        *[f"- Snapshot data quality: {warning}" for warning in snapshot_warnings],
        *[f"- Requested-scope adjustment: {warning}" for warning in scope_warnings],
    ]
    if partial:
...
def _validate_output_path(output_path: Path, issues_snapshot_path: Path, prs_snapshot_path: Path, health_report_path: Path | None) -> None:
    """Reject a report destination that would overwrite a supplied input artifact."""
    resolved_output = output_path.resolve()
    inputs = [
        ("issues snapshot", issues_snapshot_path),
        ("pull-request snapshot", prs_snapshot_path),
    ]
    ...
    for label, input_path in inputs:
        if resolved_output == input_path.resolve():
            raise ValueError(
                "Output path {0} collides with input {1} {2}.".format(resolved_output, label, input_path.resolve())
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--include-closed", action="store_true", help="Classify closed snapshot items in addition to the default open-only scope.")
    ...
    try:
        _validate_output_path(args.output, args.issues_snapshot, args.prs_snapshot, args.health_report)
        _validate_directive()
        ...
        records, snapshot_warnings, scope_warnings = triage(
            issues_snapshot, prs_snapshot, as_of, health_context, args.include_closed
        )
        report = _render_report(
            args.issues_snapshot.resolve(),
            args.prs_snapshot.resolve(),
            health_context,
            issues_snapshot,
            prs_snapshot,
            records,
