...
- A safety halt in an earlier stage (for example, no confirmed fork or a
  supplied-but-invalid draft-PR receipt) prevents that stage's action and any downstream action
...
- If no draft PR exists, state that it was not created; do not treat this as a
  pipeline failure unless the operator had approved that stage. A complete
  local preview with `mode: preview` and `status: awaiting_operator_approval`
  is the normal no-write outcome when approval was not supplied: report the
  pending authorization clearly, but do not label an otherwise complete report
  partial.
