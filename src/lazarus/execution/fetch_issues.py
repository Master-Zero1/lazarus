...
def _get_json(url: str) -> tuple[Any, dict[str, str]]:
    ...
    try:
        ...
        try:
            ...
            message = error_payload.get("message", body) if isinstance(error_payload, dict) else body
...
def _object_or_empty(value: Any) -> dict[str, Any]:
    """Return API object metadata only when it has the expected JSON shape."""
    return value if isinstance(value, dict) else {}


def _list_or_empty(value: Any) -> list[Any]:
    """Return API list metadata only when it has the expected JSON shape."""
    return value if isinstance(value, list) else []


def _issue_metadata(issue: dict[str, Any]) -> dict[str, Any]:
    user = _object_or_empty(issue.get("user"))
    labels = _list_or_empty(issue.get("labels"))
    return {
